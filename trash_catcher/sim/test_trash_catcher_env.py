...
class TestTrashCatcherEnv:

    def setup_method(self):
        self.env = TrashCatcherEnv()
    ...
    def test_catch_gives_positive_reward(self):
        ...
        self.env.robot_pos = self.env.landing_pos.copy()
    ...
    def test_timeout_gives_negative_reward(self):
        """時間切れで負の報酬か"""
        self.env.reset(seed=42)
        self.env.robot_pos = np.array([5.0, 5.0], dtype=np.float32)
        self.env.landing_pos = np.array([-5.0, -5.0], dtype=np.float32)
